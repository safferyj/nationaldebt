...
- The portrait-only assertions correspond to the application breakpoint `max-width: 760px` plus `orientation: portrait`: the page starts in kiosk mode, controls are 40px high and measure lozenges are approximately 37px high, the normal chart is 480px high after leaving kiosk mode, and the Government value uses its short form. Wider tablets and wide landscape profiles should not be expected to auto-enter kiosk mode.
